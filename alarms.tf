locals {
  ...
  insufficient_data_warn_actions = compact(
    coalescelist(var.insufficient_data_warn_actions, var.notify_warn_arns),
  )
...
module "httpcode_rate_alarm_label" {
  ...
  namespace  = var.namespace
  stage      = var.stage
  attributes = compact(concat(var.attributes, ["%v", "rate", "high"]))
}
...
resource "aws_cloudwatch_metric_alarm" "httpcode_target_4xx_count_warn" {
  ...
  alarm_description = format(
    ...
    "4XX",
    module.default_label.id,
    local.thresholds["target_4xx_count_warn"],
    var.period / 60,
    var.evaluation_periods,
  )
...
resource "aws_cloudwatch_metric_alarm" "httpcode_target_4xx_rate" {
  ...
  treat_missing_data  = var.treat_missing_data
  alarm_description = format(
    var.httpcode_rate_alarm_description,
    "4XX",
    module.default_label.id,
    local.thresholds["target_4xx_rate"],
    var.period / 60,
    var.evaluation_periods,
  )
  alarm_actions = local.alarm_actions
...
resource "aws_cloudwatch_metric_alarm" "httpcode_target_4xx_rate_warn" {
  ...
  alarm_description = format(
    ...
    "4XX",
    module.default_label.id,
    local.thresholds["target_4xx_rate_warn"],
    var.period / 60,
    var.evaluation_periods,
  )
  alarm_actions = local.alarm_warn_actions
...
resource "aws_cloudwatch_metric_alarm" "httpcode_target_5xx_count_warn" {
  ...
  alarm_description = format(
    ...
    "5XX",
    module.default_label.id,
    local.thresholds["target_5xx_count_warn"],
    var.period / 60,
    var.evaluation_periods,
  )
  alarm_actions = local.alarm_warn_actions
...
resource "aws_cloudwatch_metric_alarm" "httpcode_target_5xx_rate" {
  ...
  alarm_description = format(
    ...
    "5XX",
    module.default_label.id,
    local.thresholds["target_5xx_rate"],
    var.period / 60,
    var.evaluation_periods,
  )
  alarm_actions = local.alarm_actions
...
resource "aws_cloudwatch_metric_alarm" "httpcode_target_5xx_rate_warn" {
  ...
  alarm_description = format(
    ...
    "5XX",
    module.default_label.id,
    local.thresholds["target_5xx_rate_warn"],
    var.period / 60,
    var.evaluation_periods,
  )
  alarm_actions = local.alarm_warn_actions
...
resource "aws_cloudwatch_metric_alarm" "healthy_host_target_count" {
  ...
  treat_missing_data  = var.treat_missing_data
  alarm_description = format(
    var.target_healthy_host_alarm_description,
    module.default_label.id,
    local.thresholds["target_healthy_host_count"],
    var.period / 60,
  )
  alarm_actions = local.alarm_actions
  ok_actions = local.ok_actions
...
resource "aws_cloudwatch_metric_alarm" "healthy_host_target_count_warn" {
  ...
  alarm_description = format(
    var.target_healthy_host_alarm_description,
    module.default_label.id,
    local.thresholds["target_healthy_host_count_warn"],
    var.period / 60,
  )
  alarm_actions = local.alarm_warn_actions
  ok_actions = local.ok_warn_actions
